...
if (isset($_POST['save-date'])) {
    ...
    foreach ($_POST['sort'] as $key => $img) {
        $key = $key + 1;
        $sql = "UPDATE `services-photos` SET `sort` = '" . $key . "'  WHERE id = '" . $img . "'";
        $db = new DB();
        $db->readQuery($sql);
    }
}

$servicephotos = getAllServicePhotos($id);
...
<div class="content-wrapper">
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-12">
                <h1 class="page-head-line">Arrange Activities Images </h1>
            </div>
        </div>
        <!--end banner-->
                                    ...
                                    <div class="col-sm-12">
                                        <ul id="sortable">
                                            ...
                                            if (count($servicephotos) > 0) {
                                                foreach ($servicephotos as $key => $img) {
                                                    ?>
                                                    <li class="ui-state-default">
                                                        <span class="number-class">(<?php echo $key + 1; ?>)</span>
                                                        <img width="300px" height="130" class="example-image img-responsive" src="../images/services/gallery/thumb/<?php echo $img["image_name"]; ?>" alt=""/>
