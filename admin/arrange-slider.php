...
if (isset($_POST['save-date'])) {
    ...
    foreach ($_POST['sort'] as $key => $img) {
        $key = $key + 1;
        $sql = "UPDATE `slider` SET `sort` = '" . $key . "'  WHERE id = '" . $img . "'";
        $db = new DB();
        $db->readQuery($sql);
    }
}
...
<div class="content-wrapper">
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-12">
                <h1 class="page-head-line">Arrange Slider </h1>
            </div>
        </div>
        <!--end banner-->
                                    ...
                                    <div class="col-sm-12">
                                        <ul id="sortable">
                                            ...
                                            if (count($sliderPhotos) > 0) {
                                                foreach ($sliderPhotos as $key => $img) {
                                                    ?>
                                                    <li class="ui-state-default">
                                                        <span class="number-class">(<?php echo $key + 1; ?>)</span>
                                                        <img width="300px" height="130" class="example-image img-responsive" src="../images/slider/<?php echo $img["image_name"]; ?>" alt=""/>
